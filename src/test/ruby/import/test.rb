...
class DiffJImportsTestCase < DiffJTestCase
  ...
  def test_section_added
    run_imp_test 'SctnAdded', section_added(loc(1, 1), loc(1, 5), loc(1, 1), loc(2, 30))
  end

  def test_section_removed
    run_imp_test 'SctnRemoved', section_removed(loc(1, 1), loc(2, 32), loc(1, 1), loc(1, 5))
  end

  def test_import_added
    run_imp_test 'Added', import_added('org.incava.Added', loc(1, 1), loc(1, 27), loc(2, 1), loc(2, 24))
  end
